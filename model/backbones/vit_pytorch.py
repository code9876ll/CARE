...
class PatchEmbed_overlap(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        # print('x.shape:',x.shape)#[16,3,256,128]
        # FIXME look at relaxing size constraints
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x)
        # print('x_proj:',x.shape)#[16,768,16,8]
        x = x.flatten(2).transpose(1, 2)
        # print('flatten后x.shape:', x.shape)#[16,128,768]
        return x
...
class TransReID(nn.Module):
    ...
    def forward_features(self, x, camera_id, view_id):
        B = x.shape[0]
        # print('x.shape:',x.shape)#[32, 3, 256, 128]
        x = self.patch_embed(x)
        # print('x2.shape:', x.shape)#[32, 128, 768]

        cls_tokens = self.cls_token.expand(B, -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        x = torch.cat((cls_tokens, x), dim=1)
        # print('与cls cat后',x.shape)#[16,129,768]
        ...
        x = self.pos_drop(x)
        # print('pos_drop后', x.shape)#[16,129,768]
        if self.local_feature:
            ...
            for blk in self.blocks:
                x = blk(x)
            # print('blocks', x.shape)#[16,129,768]
            x = self.norm(x)# 这个应该是在ViT之外自己另外加的layer norm
            # print('x_norm',x.shape)#[16,129,768]
            # print('x[:, 0]', x[:, 0].shape)#[16,768] #选cls token作为前传特征
            return x[:, 0]
            # return x, x[:, 0]
